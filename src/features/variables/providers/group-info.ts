...
async function fetchGroupInfo(session: Session): Promise<GroupInfo | null> {
  ...
  try {
    if (typeof internal.getGroupInfo === 'function') {
      return await (internal.getGroupInfo as (groupId: number, noCache?: boolean) => Promise<GroupInfo>)(groupId, false)
    }
    if (typeof internal._request === 'function') {
      return await (internal._request as (action: string, params: unknown) => Promise<GroupInfo>)('get_group_info', {
        group_id: groupId,
        no_cache: false,
      })
    }
...
async function fetchOwnersAndAdmins(session: Session, log?: LogFn): Promise<{ owners: string[]; admins: string[] } | null> {
  try {
    if (session.platform !== 'onebot') return null
    ...
    if (internal) {
      if (typeof internal.getGroupMemberList === 'function') {
        members = await (internal.getGroupMemberList as (groupId: string | number) => Promise<MemberInfo[]>)(guildId)
      } else if (typeof internal._request === 'function') {
        members = await (internal._request as (action: string, params: unknown) => Promise<MemberInfo[]>)('get_group_member_list', {
    ...
    for (const member of members) {
      ...
      const candidates = [member.card, member.remark, member.displayName, member.nick, member.nickname, member.name].map((item) =>
...
export function createGroupInfoProvider(deps: GroupInfoProviderDeps) {
  ...
  return async (_args: unknown, _variables: unknown, configurable?: ProviderConfigurable): Promise<string> => {
