...
export const NativeToolsSchema = Schema.object({
  ...
  enablePokeTool: Schema.boolean().default(false).description('注册 ChatLuna 工具：戳一戳（建议与 XML工具 二选一）'),
  ...
  enableSetMsgEmojiTool: Schema.boolean().default(false).description('注册 ChatLuna 工具：给消息添加表情（需 chatluna-character 开启 enableMessageId，建议与 XML工具 二选一）'),
  setMsgEmojiToolName: Schema.string().default('set_msg_emoji').description('ChatLuna 工具名称：给消息添加表情'),
  enableDeleteMessageTool: Schema.boolean().default(false).description('注册 ChatLuna 工具：撤回消息（需 chatluna-character 开启 enableMessageId，建议与 XML工具 二选一）'),
