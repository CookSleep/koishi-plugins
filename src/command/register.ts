...
interface ChatlunaCharacterLoggerLike {
...
export function registerCommands(ctx: Context, config: Config): void {
  ...
  const handleXmlMemeToolCall = async (
    ...
    try {
      ...
      if (isExcludedMemeKey(resolvedKey, mergedExcludedMemeKeySet())) {
        return handleErrorReply("meme.xml", "该模板已被排除。");
      }
  ...
  if (config.enableMemeXmlTool) {
    ...
    let rawInterceptorOriginalDebug: ((...args: unknown[]) => void) | null =
      null;
    ...
    const initRawModelInterceptor = (): boolean => {
      ...
      if (
        !rawCollectorBound &&
        typeof characterService.collect === "function"
      ) {
      ...
      (wrappedDebug as unknown as Record<string, boolean>)[
        RAW_INTERCEPTOR_TAG
      ] = true;
